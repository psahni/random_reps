...
class PoolsControllerTest < ActionController::TestCase
  ...
  test "should create pool" do
    assert_difference('Pool.count') do
      post :create, pool: { hostname: @pool.hostname, password: @pool.password, port: @pool.port, username: @pool.username }
    end
  ...
  test "should update pool" do
    patch :update, id: @pool, pool: { hostname: @pool.hostname, password: @pool.password, port: @pool.port, username: @pool.username }
    assert_redirected_to pool_path(assigns(:pool))
  end
